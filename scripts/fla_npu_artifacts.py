...
def get_branch_name(repo_root: Path) -> str:
    ...
    if normalized_current == "master" or re.match(r"^v\d", normalized_current):
...
def _branch_build_tag(repo_root: Path, public_version: str) -> str:
    branch_tag = _wheel_tag_part(get_branch_name(repo_root))
    stable_branch_tags = {
        _wheel_tag_part(public_version),
        _wheel_tag_part(f"v{public_version}"),
    }
    if branch_tag and branch_tag not in stable_branch_tags:
        return branch_tag
    return ""
...
def get_wheel_build_tag(repo_root: Path, public_version: str | None = None) -> str:
    ...
    build_parts = []
    branch_tag = _branch_build_tag(repo_root, public_version)
    if branch_tag:
        build_parts.append(branch_tag)
    build_parts.extend([product_tag, arch_tag])
    build_tag = ".".join(build_parts)
    if build_tag and not build_tag[0].isdigit():
...
def get_package_version(repo_root: Path) -> str:
    return read_public_version(repo_root)


def get_wheel_filename(repo_root: Path) -> str:
    public_version = get_package_version(repo_root)
    build_tag = get_wheel_build_tag(repo_root, public_version)
    if build_tag:
        return f"{WHEEL_DIST_NAME}-{public_version}-{build_tag}-py3-none-any.whl"
    return f"{WHEEL_DIST_NAME}-{public_version}-py3-none-any.whl"
