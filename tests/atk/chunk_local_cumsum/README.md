# ChunkLocalCumsum ATK 精度测试归档

本目录归档 `chunk_local_cumsum` 算子的 ATK 看护资产。算子仅覆盖 A2
...
| 项 | 值 |
| --- | --- |
| 公开 Python API | `fla_npu.ops.ascendc.chunk_local_cumsum` |
| ACLNN 名称 | `ChunkLocalCumsum` |
| 源算子 | `fla/ops/ascendc/gdn/chunk_gdn_fwd/chunk_local_cumsum` |
| SOC | `ascend910b`（A2） |
| route | `ascendc` |
...
| case ID | seed | B | H | T | chunk_size | dtype | output_dtype | reverse | head_first | varlen | cu_seqlens | explicit_chunk_indices | tags |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 0 | 20260813 | 1 | 8 | 128 | 64 | fp32 | float32 | false | true | false | "" | false | accuracy,smoke |
| 1 | 20260814 | 1 | 8 | 256 | 64 | bf16 | float32 | true | true | true | "0,128,256" | true | accuracy,smoke |

两条 case 共享 `B=1`、`H=8`、`chunk_size=64`、`output_dtype=float32`、
`head_first=true`。case 0 覆盖固定长度、`dtype=fp32`、正向累加（`reverse=false`）；
case 1 覆盖 varlen（`cu_seqlens="0,128,256"`）、`dtype=bf16`、反向累加（`reverse=true`）、
显式 chunk 索引，分别验证两种 dtype 与正/反向累加下的局部 chunk 前缀和输出。

## 2. 文件

```text
atk_chunk_local_cumsum.json
chunk_local_cumsum.yaml
executor_chunk_local_cumsum.py
gen_chunk_local_cumsum.py
README.md
```

`executor_chunk_local_cumsum.py` 注册为 `executor_chunk_local_cumsum`，复用
...
cd "$REPO_ROOT/test/chunk_local_cumsum"
atk --version
python -c 'import fla_npu; from fla_npu.ops.ascendc import chunk_local_cumsum; print(fla_npu.__file__)'
...
bash build.sh --pkg --soc=ascend910b \
  --vendor_name=fla_npu --ops=chunk_local_cumsum
```
...
atk node --name npu_dut --backend npu --devices 0 \
  ...
  node --name cpu_reference --backend cpu \
    --output_path ./atk_output/cpu_dual_reference \
  task \
    -c ./atk_chunk_local_cumsum.json \
    --task accuracy \
    --bm_device cpu \
    -p ./executor_chunk_local_cumsum.py \
    -s 0 \
    -e 2 \
    -sp \
    -mt 1 \
...
cd "$REPO_ROOT/test/chunk_local_cumsum"
atk case \
  -f ./chunk_local_cumsum.yaml \
  -p ./gen_chunk_local_cumsum.py \
  -dt 1 \
  -en 0 \
  -s 20260813

python3 ./gen_chunk_local_cumsum.py \
  --output ./atk_chunk_local_cumsum.generated.json \
  --summary
```
...
| NPU 报算子不存在 | 未构建/安装匹配 SOC 的 OPP 包。按第 3.1 节构建 `ascend910b` 的 `chunk_local_cumsum` OPP。 |
...
| 生成结果与已评审 JSON 不一致 | 比较生成器、YAML 和 seed；不要静默覆盖 `atk_chunk_local_cumsum.json`。 |
