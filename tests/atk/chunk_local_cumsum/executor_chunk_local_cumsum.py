"""ATK executor for chunk_local_cumsum."""
...
def prepare_call_args(spec: dict, low_marker: torch.Tensor):
    ...
    g = _normal((int(spec.get("B", 1)), int(spec.get("H", 8)), int(spec.get("T", 128))), _dtype(spec.get("dtype", "fp32")), low_marker.device, _seed(spec, 1), scale=0.02)
    return g, int(spec.get("chunk_size", 64)), _cu(spec), _chunk_indices(spec), _as_bool(spec.get("reverse", False)), float(spec.get("scale", 1.0)), _as_bool(spec.get("head_first", True)), str(spec.get("output_dtype", "float32"))


def reference_outputs(spec: dict, low_marker: torch.Tensor) -> tuple[torch.Tensor, ...]:
    device = low_marker.device
    g = _normal((int(spec.get("B", 1)), int(spec.get("H", 8)), int(spec.get("T", 128))), _dtype(spec.get("dtype", "fp32")), device, _seed(spec, 1), scale=0.02)
    return (torch.cumsum(g.to(torch.float32) * float(spec.get("scale", 1.0)), dim=-1),)
...
def run_npu(public_api: str, spec: dict, low_marker: torch.Tensor):
    ...
    return op(args[0], args[1], cu_seqlens=args[2], chunk_indices_out=args[3], reverse=args[4], scale=args[5], head_first=args[6], output_dtype=args[7])
