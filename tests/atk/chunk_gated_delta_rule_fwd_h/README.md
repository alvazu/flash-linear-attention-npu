# ChunkGatedDeltaRuleFwdH ATK 精度测试归档

本目录归档 `chunk_gated_delta_rule_fwd_h` 算子的 ATK 看护资产。算子仅覆盖 A2
...
| 项 | 值 |
| --- | --- |
| 公开 Python API | `fla_npu.ops.ascendc.chunk_gated_delta_rule_fwd_h` |
| ACLNN 名称 | `ChunkGatedDeltaRuleFwdH` |
| 源算子 | `fla/ops/ascendc/gdn/chunk_gdn_fwd/chunk_gated_delta_rule_fwd_h` |
| SOC | `ascend910b`（A2） |
| route | `ascendc` |
...
| case ID | seed | B | H | HV | T | K | V | chunk_size | dtype | g_dtype | varlen | cu_seqlens | explicit_chunk_indices | output_final_state | state_v_first | tags |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 0 | 20260813 | 1 | 4 | 4 | 128 | 128 | 128 | 64 | bf16 | fp32 | false | "" | false | true | false | accuracy,smoke |
| 1 | 20260814 | 1 | 4 | 8 | 256 | 128 | 128 | 64 | fp16 | bf16 | true | "0,128,256" | true | true | false | accuracy,smoke |

两条 case 共享 `B=1`、`H=4`、`K=V=128`、`chunk_size=64`、`output_final_state=true`、
`state_v_first=false`。case 0 覆盖固定长度、`HV=4`、`g_dtype=fp32`；case 1 覆盖 varlen
（`cu_seqlens="0,128,256"`）、`HV=8`、`g_dtype=bf16`、显式 chunk 索引，
分别验证两种 dtype 与两种序列组织下的 chunk 前向 `h` 状态输出。

## 2. 文件

```text
atk_chunk_gated_delta_rule_fwd_h.json
chunk_gated_delta_rule_fwd_h.yaml
executor_chunk_gated_delta_rule_fwd_h.py
gen_chunk_gated_delta_rule_fwd_h.py
README.md
```

`executor_chunk_gated_delta_rule_fwd_h.py` 注册为 `executor_chunk_gated_delta_rule_fwd_h`，复用
...
cd "$REPO_ROOT/test/chunk_gated_delta_rule_fwd_h"
atk --version
python -c 'import fla_npu; from fla_npu.ops.ascendc import chunk_gated_delta_rule_fwd_h; print(fla_npu.__file__)'
...
bash build.sh --pkg --soc=ascend910b \
  --vendor_name=fla_npu --ops=chunk_gated_delta_rule_fwd_h
```
...
atk node --name npu_dut --backend npu --devices 0 \
  ...
  node --name cpu_reference --backend cpu \
    --output_path ./atk_output/cpu_dual_reference \
  task \
    -c ./atk_chunk_gated_delta_rule_fwd_h.json \
    --task accuracy \
    --bm_device cpu \
    -p ./executor_chunk_gated_delta_rule_fwd_h.py \
    -s 0 \
    -e 2 \
    -sp \
    -mt 1 \
...
cd "$REPO_ROOT/test/chunk_gated_delta_rule_fwd_h"
atk case \
  -f ./chunk_gated_delta_rule_fwd_h.yaml \
  -p ./gen_chunk_gated_delta_rule_fwd_h.py \
  -dt 1 \
  -en 0 \
  -s 20260813

python3 ./gen_chunk_gated_delta_rule_fwd_h.py \
  --output ./atk_chunk_gated_delta_rule_fwd_h.generated.json \
  --summary
```
...
ct viz \
  ...
  --name chunk_gated_delta_rule_fwd_h_out \
  --spatial
```
...
| NPU 报算子不存在 | 未构建/安装匹配 SOC 的 OPP 包。按第 3.1 节构建 `ascend910b` 的 `chunk_gated_delta_rule_fwd_h` OPP。 |
...
| 生成结果与已评审 JSON 不一致 | 比较生成器、YAML 和 seed；不要静默覆盖 `atk_chunk_gated_delta_rule_fwd_h.json`。 |
