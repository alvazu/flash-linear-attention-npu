"""ATK executor for chunk_bwd_dv_local."""
...
def prepare_call_args(spec: dict, low_marker: torch.Tensor):
    ...
    do = _rand(v.shape, _dtype(spec.get("dtype", "bf16")), device, _seed(spec, 6))
    a = _rand((int(spec.get("chunk_size", 64)), int(spec.get("chunk_size", 64))), _dtype(spec.get("dtype", "bf16")), device, _seed(spec, 7))
    scale = float(spec.get("scale", 1.0 / math.sqrt(int(spec.get("K", 128)))))
    return q, k, do, g, scale, int(spec.get("chunk_size", 64)), None, a, _cu(spec), _chunk_indices(spec)
...
def run_npu(public_api: str, spec: dict, low_marker: torch.Tensor):
    ...
    return op(*args[:6], g_gamma=args[6], A=args[7], cu_seqlens=args[8], chunk_indices=args[9])
