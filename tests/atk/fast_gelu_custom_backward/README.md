# FastGeluBackward ATK 精度测试归档

本目录归档 `fast_gelu_custom_backward` 算子的 ATK 看护资产。算子仅覆盖 A2
...
| 项 | 值 |
| --- | --- |
| 公开 Python API | `fla_npu.ops.ascendc.fast_gelu_custom_backward` |
| ACLNN 名称 | `FastGeluBackward` |
...
下的 FastGelu 反向梯度输出。case 矩阵与正向 `fast_gelu_custom` 一一对应，使用
相同 shape 和 seed，便于前后向联动复检。

## 2. 文件

```text
atk_fast_gelu_custom_backward.json
fast_gelu_custom_backward.yaml
executor_fast_gelu_custom_backward.py
gen_fast_gelu_custom_backward.py
README.md
```

`executor_fast_gelu_custom_backward.py` 注册为 `executor_fast_gelu_custom_backward`，复用
...
cd "$REPO_ROOT/test/fast_gelu_custom_backward"
atk --version
python -c 'import fla_npu; from fla_npu.ops.ascendc import fast_gelu_custom_backward; print(fla_npu.__file__)'
...
bash build.sh --pkg --soc=ascend910b \
  --vendor_name=fla_npu --ops=fast_gelu_custom_backward
```
...
atk node --name npu_dut --backend npu --devices 0 \
  ...
  node --name cpu_reference --backend cpu \
    --output_path ./atk_output/cpu_dual_reference \
  task \
    -c ./atk_fast_gelu_custom_backward.json \
    --task accuracy \
    --bm_device cpu \
    -p ./executor_fast_gelu_custom_backward.py \
    -s 0 \
    -e 3 \
    -sp \
    -mt 1 \
...
cd "$REPO_ROOT/test/fast_gelu_custom_backward"
atk case \
  -f ./fast_gelu_custom_backward.yaml \
  -p ./gen_fast_gelu_custom_backward.py \
  -dt 1 \
  -en 0 \
  -s 20260813

python3 ./gen_fast_gelu_custom_backward.py \
  --output ./atk_fast_gelu_custom_backward.generated.json \
  --summary
```
...
ct viz \
  ...
  --name fast_gelu_custom_backward_out \
  --spatial
```
...
| NPU 报算子不存在 | 未构建/安装匹配 SOC 的 OPP 包。按第 3.1 节构建 `ascend910b` 的 `fast_gelu_custom_backward` OPP。 |
...
| 生成结果与已评审 JSON 不一致 | 比较生成器、YAML 和 seed；不要静默覆盖 `atk_fast_gelu_custom_backward.json`。 |
