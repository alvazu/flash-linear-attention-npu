# RecurrentKda ATK 精度测试归档

本目录归档 `recurrent_kda` 算子的 ATK 看护资产。算子仅覆盖 A2
...
| 项 | 值 |
| --- | --- |
| 公开 Python API | `fla_npu.ops.ascendc.recurrent_kda` |
| ACLNN 名称 | `RecurrentKda` |
| 源算子 | `fla/ops/ascendc/kda/recurrent_kda` |
| SOC | `ascend910b`（A2） |
| route | `ascendc` |
...
| case ID | seed | B | H | HV | K | V | T | dtype | layout | state_v_first | varlen | cu_seqlens | explicit_chunk_indices | tags |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 0 | 20260813 | 1 | 4 | 4 | 128 | 128 | 128 | bf16 | BSND | false | false | "" | false | accuracy,smoke |
| 1 | 20260814 | 1 | 4 | 8 | 128 | 128 | 256 | bf16 | TND | true | true | "0,128,256" | true | accuracy,smoke |

两条 case 共享 `B=1`、`H=4`、`K=128`、`V=128`、`dtype=bf16`。case 0 覆盖固定长度、
`HV=4`、`layout=BSND`、`state_v_first=false`；case 1 覆盖 varlen
（`cu_seqlens="0,128,256"`）、`HV=8`、`layout=TND`、`state_v_first=true`、显式 chunk 索引，
分别验证两种 layout 与 state_v_first 开关下 KDA 递归前向输出。

## 2. 文件

```text
atk_recurrent_kda.json
recurrent_kda.yaml
executor_recurrent_kda.py
gen_recurrent_kda.py
README.md
```

`executor_recurrent_kda.py` 注册为 `executor_recurrent_kda`，复用
...
cd "$REPO_ROOT/test/recurrent_kda"
atk --version
python -c 'import fla_npu; from fla_npu.ops.ascendc import recurrent_kda; print(fla_npu.__file__)'
...
bash build.sh --pkg --soc=ascend910b \
  --vendor_name=fla_npu --ops=recurrent_kda
```
...
cd "$REPO_ROOT/test/recurrent_kda"
atk case \
  -f ./recurrent_kda.yaml \
  -p ./gen_recurrent_kda.py \
  -dt 1 \
  -en 0 \
  -s 20260813

python3 ./gen_recurrent_kda.py \
  --output ./atk_recurrent_kda.generated.json \
  --summary
```

相同 YAML、gen 和 seed 必须生成稳定 case ID 与稳定结构。生成后检查 ATK schema、
case 数量、SOC、route、dtype、layout、state_v_first、varlen 分布和覆盖摘要，不要静默覆盖
已评审的 `atk_recurrent_kda.json`。
...
| NPU 报算子不存在 | 未构建/安装匹配 SOC 的 OPP 包。按第 3.1 节构建 `ascend910b` 的 `recurrent_kda` OPP。 |
...
| 生成结果与已评审 JSON 不一致 | 比较生成器、YAML 和 seed；不要静默覆盖 `atk_recurrent_kda.json`。 |
