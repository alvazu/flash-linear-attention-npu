# ChunkFwdO ATK 精度测试归档

本目录归档 `chunk_fwd_o` 算子的 ATK 看护资产。算子仅覆盖 A2
...
| 项 | 值 |
| --- | --- |
| 公开 Python API | `fla_npu.ops.ascendc.chunk_fwd_o` |
| ACLNN 名称 | `ChunkFwdO` |
| 源算子 | `fla/ops/ascendc/gdn/chunk_gdn_fwd/chunk_fwd_o` |
| SOC | `ascend910b`（A2） |
| route | `ascendc` |
...
| case ID | seed | B | H | HV | T | K | V | chunk_size | dtype | g_dtype | varlen | cu_seqlens | explicit_chunk_indices | tags |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 0 | 20260813 | 1 | 4 | 4 | 128 | 128 | 128 | 64 | bf16 | fp32 | false | "" | false | accuracy,smoke |
| 1 | 20260814 | 1 | 4 | 8 | 256 | 128 | 128 | 64 | fp16 | bf16 | true | "0,128,256" | true | accuracy,smoke |

两条 case 共享 `B=1`、`H=4`、`K=V=128`、`chunk_size=64`。case 0
覆盖固定长度、`HV=4`、`g_dtype=fp32`；case 1 覆盖 varlen
（`cu_seqlens="0,128,256"`）、`HV=8`、`g_dtype=bf16`、显式 chunk 索引，
分别验证两种 dtype 与两种序列组织下的 chunk 前向 `o` 输出。

## 2. 文件

```text
atk_chunk_fwd_o.json
chunk_fwd_o.yaml
executor_chunk_fwd_o.py
gen_chunk_fwd_o.py
README.md
```

`executor_chunk_fwd_o.py` 注册为 `executor_chunk_fwd_o`，复用
...
cd "$REPO_ROOT/test/chunk_fwd_o"
atk --version
python -c 'import fla_npu; from fla_npu.ops.ascendc import chunk_fwd_o; print(fla_npu.__file__)'
...
bash build.sh --pkg --soc=ascend910b \
  --vendor_name=fla_npu --ops=chunk_fwd_o
```
...
atk node --name npu_dut --backend npu --devices 0 \
  ...
  node --name cpu_reference --backend cpu \
    --output_path ./atk_output/cpu_dual_reference \
  task \
    -c ./atk_chunk_fwd_o.json \
    --task accuracy \
    --bm_device cpu \
    -p ./executor_chunk_fwd_o.py \
    -s 0 \
    -e 2 \
    -sp \
    -mt 1 \
...
atk case \
  -f ./chunk_fwd_o.yaml \
  -p ./gen_chunk_fwd_o.py \
  -dt 1 \
  -en 0 \
  -s 20260813

python3 ./gen_chunk_fwd_o.py \
  --output ./atk_chunk_fwd_o.generated.json \
  --summary
```
...
| NPU 报算子不存在 | 未构建/安装匹配 SOC 的 OPP 包。按第 3.1 节构建 `ascend910b` 的 `chunk_fwd_o` OPP。 |
...
| 生成结果与已评审 JSON 不一致 | 比较生成器、YAML 和 seed；不要静默覆盖 `atk_chunk_fwd_o.json`。 |
