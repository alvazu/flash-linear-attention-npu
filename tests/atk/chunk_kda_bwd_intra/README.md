# ChunkKdaBwdIntra ATK 精度测试归档

本目录归档 `chunk_kda_bwd_intra` 算子的 ATK 看护资产。算子仅覆盖 A2
...
| 项 | 值 |
| --- | --- |
| 公开 Python API | `fla_npu.ops.ascendc.chunk_kda_bwd_intra` |
| ACLNN 名称 | `ChunkKdaBwdIntra` |
| 源算子 | `fla/ops/ascendc/kda/chunk_kda_bwd_intra` |
| SOC | `ascend910b`（A2） |
| route | `ascendc` |
...
| case ID | seed | B | H | K | T | chunk_size | dtype | beta_dtype | layout | varlen | cu_seqlens | explicit_chunk_indices | tags |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 0 | 20260813 | 1 | 4 | 128 | 128 | 64 | bf16 | fp32 | BSND | false | "" | false | accuracy,smoke |
| 1 | 20260814 | 1 | 8 | 128 | 256 | 64 | bf16 | bf16 | TND | true | "0,128,256" | true | accuracy,smoke |

两条 case 共享 `B=1`、`K=128`、`chunk_size=64`、`dtype=bf16`。case 0
覆盖固定长度、`H=4`、`beta_dtype=fp32`、`layout=BSND`；case 1 覆盖 varlen
（`cu_seqlens="0,128,256"`）、`H=8`、`beta_dtype=bf16`、`layout=TND`、显式 chunk 索引，
分别验证两种 `beta` 精度与两种 layout 下的 intra-chunk 反传梯度。

## 2. 文件

```text
atk_chunk_kda_bwd_intra.json
chunk_kda_bwd_intra.yaml
executor_chunk_kda_bwd_intra.py
gen_chunk_kda_bwd_intra.py
README.md
```

`executor_chunk_kda_bwd_intra.py` 注册为 `executor_chunk_kda_bwd_intra`，复用
...
cd "$REPO_ROOT/test/chunk_kda_bwd_intra"
atk --version
python -c 'import fla_npu; from fla_npu.ops.ascendc import chunk_kda_bwd_intra; print(fla_npu.__file__)'
...
bash build.sh --pkg --soc=ascend910b \
  --vendor_name=fla_npu --ops=chunk_kda_bwd_intra
```
...
atk node --name npu_dut --backend npu --devices 0 \
  ...
  node --name cpu_reference --backend cpu \
    --output_path ./atk_output/cpu_dual_reference \
  task \
    -c ./atk_chunk_kda_bwd_intra.json \
    --task accuracy \
    --bm_device cpu \
    -p ./executor_chunk_kda_bwd_intra.py \
    -s 0 \
    -e 2 \
    -sp \
    -mt 1 \
...
cd "$REPO_ROOT/test/chunk_kda_bwd_intra"
atk case \
  -f ./chunk_kda_bwd_intra.yaml \
  -p ./gen_chunk_kda_bwd_intra.py \
  -dt 1 \
  -en 0 \
  -s 20260813

python3 ./gen_chunk_kda_bwd_intra.py \
  --output ./atk_chunk_kda_bwd_intra.generated.json \
  --summary
```
...
| NPU 报算子不存在 | 未构建/安装匹配 SOC 的 OPP 包。按第 3.1 节构建 `ascend910b` 的 `chunk_kda_bwd_intra` OPP。 |
...
| 生成结果与已评审 JSON 不一致 | 比较生成器、YAML 和 seed；不要静默覆盖 `atk_chunk_kda_bwd_intra.json`。 |
