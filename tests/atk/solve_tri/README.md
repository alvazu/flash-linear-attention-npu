...
本目录归档 `solve_tri` 算子的 ATK 看护资产。算子仅覆盖 A2
...
| 项 | 值 |
| --- | --- |
| 公开 Python API | `fla_npu.ops.ascendc.solve_tri` |
| ACLNN 名称 | `SolveTri` |
| 源算子 | `fla/ops/ascendc/gdn/chunk_gdn_fwd/solve_tri` |
| SOC | `ascend910b`（A2） |
| route | `ascendc` |
...
| case ID | seed | T | chunk_size | dtype | layout | varlen | cu_seqlens | explicit_chunk_indices | tags |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 0 | 20260813 | 128 | 64 | fp32 | bsnd | false | "" | false | accuracy,smoke |
| 1 | 20260814 | 128 | 64 | fp32 | tnd | true | "0,64,128" | true | accuracy,smoke |

两条 case 共享 `T=128`、`chunk_size=64`、`dtype=fp32`。case 0 覆盖固定长度、
`layout=bsnd`；case 1 覆盖 varlen（`cu_seqlens="0,64,128"`）、`layout=tnd`、
显式 chunk 索引，分别验证两种 layout 与固定/varlen 序列下的三角求解输出。
算子仅覆盖 `dtype=fp32`，bf16/fp16 不在冻结范围。
...
atk_solve_tri.json
solve_tri.yaml
executor_solve_tri.py
gen_solve_tri.py
README.md
```

`executor_solve_tri.py` 注册为 `executor_solve_tri`，复用
...
cd "$REPO_ROOT/test/solve_tri"
atk --version
python -c 'import fla_npu; from fla_npu.ops.ascendc import solve_tri; print(fla_npu.__file__)'
...
bash build.sh --pkg --soc=ascend910b \
  --vendor_name=fla_npu --ops=solve_tri
```
...
atk node --name npu_dut --backend npu --devices 0 \
  ...
  node --name cpu_reference --backend cpu \
    --output_path ./atk_output/cpu_dual_reference \
  task \
    -c ./atk_solve_tri.json \
    --task accuracy \
    --bm_device cpu \
    -p ./executor_solve_tri.py \
    -s 0 \
    -e 2 \
    -sp \
    -mt 1 \
...
python3 ./gen_solve_tri.py \
  --output ./atk_solve_tri.generated.json \
  --summary
```

相同 YAML、gen 和 seed 必须生成稳定 case ID 与稳定结构。生成后检查 ATK schema、
case 数量、SOC、route、dtype、layout、chunk 边界、varlen 分布和覆盖摘要，不要静默覆盖
已评审的 `atk_solve_tri.json`。
...
| NPU 报算子不存在 | 未构建/安装匹配 SOC 的 OPP 包。按第 3.1 节构建 `ascend910b` 的 `solve_tri` OPP。 |
...
| 生成结果与已评审 JSON 不一致 | 比较生成器、YAML 和 seed；不要静默覆盖 `atk_solve_tri.json`。 |
