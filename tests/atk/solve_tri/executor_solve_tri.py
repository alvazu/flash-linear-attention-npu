...
def prepare_call_args(spec: dict, low_marker: torch.Tensor):
    ...
    x = torch.tril(_normal((chunk_size, chunk_size), torch.float32, device, _seed(spec, 1), scale=0.02))
    x = x + torch.eye(chunk_size, device=device, dtype=torch.float32)
    return x, _cu(spec), _chunk_indices(spec), layout


def reference_outputs(spec: dict, low_marker: torch.Tensor) -> tuple[torch.Tensor, ...]:
    device = low_marker.device
    return (torch.eye(int(spec.get("chunk_size", 64)), device=device, dtype=torch.float32),)
...
def run_npu(public_api: str, spec: dict, low_marker: torch.Tensor):
    ...
    return op(args[0], cu_seqlens=args[1], chunk_indices=args[2], layout=args[3])
