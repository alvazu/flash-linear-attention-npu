# PrepareWyReprBwdDa ATK 精度测试归档

本目录归档 `prepare_wy_repr_bwd_da` 算子的 ATK 看护资产。算子仅覆盖 A2
...
| 项 | 值 |
| --- | --- |
| 公开 Python API | `fla_npu.ops.ascendc.prepare_wy_repr_bwd_da` |
| ACLNN 名称 | `PrepareWyReprBwdDa` |
| 源算子 | `fla/ops/ascendc/gdn/chunk_gdn_bwd/prepare_wy_repr_bwd_da` |
| SOC | `ascend910b`（A2） |
| route | `ascendc` |
...
分别验证两种 dtype/beta_dtype 组合与固定/varlen 序列下的 WY 表示反向 `da` 梯度输出。

## 2. 文件

```text
atk_prepare_wy_repr_bwd_da.json
prepare_wy_repr_bwd_da.yaml
executor_prepare_wy_repr_bwd_da.py
gen_prepare_wy_repr_bwd_da.py
README.md
```

`executor_prepare_wy_repr_bwd_da.py` 注册为 `executor_prepare_wy_repr_bwd_da`，复用
...
cd "$REPO_ROOT/test/prepare_wy_repr_bwd_da"
atk --version
python -c 'import fla_npu; from fla_npu.ops.ascendc import prepare_wy_repr_bwd_da; print(fla_npu.__file__)'
...
bash build.sh --pkg --soc=ascend910b \
  --vendor_name=fla_npu --ops=prepare_wy_repr_bwd_da
```
...
atk node --name npu_dut --backend npu --devices 0 \
  ...
  node --name cpu_reference --backend cpu \
    --output_path ./atk_output/cpu_dual_reference \
  task \
    -c ./atk_prepare_wy_repr_bwd_da.json \
    --task accuracy \
    --bm_device cpu \
    -p ./executor_prepare_wy_repr_bwd_da.py \
    -s 0 \
    -e 2 \
    -sp \
    -mt 1 \
...
cd "$REPO_ROOT/test/prepare_wy_repr_bwd_da"
atk case \
  -f ./prepare_wy_repr_bwd_da.yaml \
  -p ./gen_prepare_wy_repr_bwd_da.py \
  -dt 1 \
  -en 0 \
  -s 20260813

python3 ./gen_prepare_wy_repr_bwd_da.py \
  --output ./atk_prepare_wy_repr_bwd_da.generated.json \
  --summary
```
...
已评审的 `atk_prepare_wy_repr_bwd_da.json`。
...
ct viz \
  ...
  --name prepare_wy_repr_bwd_da_out \
  --spatial
```
...
| NPU 报算子不存在 | 未构建/安装匹配 SOC 的 OPP 包。按第 3.1 节构建 `ascend910b` 的 `prepare_wy_repr_bwd_da` OPP。 |
...
| 生成结果与已评审 JSON 不一致 | 比较生成器、YAML 和 seed；不要静默覆盖 `atk_prepare_wy_repr_bwd_da.json`。 |
