...
def prepare_call_args(spec: dict, low_marker: torch.Tensor):
    ...
    return k, v, beta, a, dw, du, g, int(spec.get("chunk_size", 64)), _cu(spec), _chunk_indices(spec)
...
def reference_outputs(spec: dict, low_marker: torch.Tensor) -> tuple[torch.Tensor, ...]:
    ...
    h = int(spec.get("H", 4))
    t = int(spec.get("T", 128))
    return (torch.zeros((b, h, t, int(spec.get("chunk_size", 64))), device=device, dtype=dtype),)
...
def run_npu(public_api: str, spec: dict, low_marker: torch.Tensor):
    ...
    return op(*args[:7], chunk_size=args[7], cu_seqlens=args[8], chunk_indices=args[9])
...
class FunctionApi(BaseApi):
    ...
    public_api = "prepare_wy_repr_bwd_da"
