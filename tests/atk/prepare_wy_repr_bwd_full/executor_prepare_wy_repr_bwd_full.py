...
def prepare_call_args(spec: dict, low_marker: torch.Tensor):
    ...
    _q, k, v, g, beta = _layout_qkv(spec, device)
    h = int(spec.get("H", 4))
    t = int(spec.get("T", 128))
    c = int(spec.get("chunk_size", 64))
    dtype = _dtype(spec.get("dtype", "bf16"))
    a = _rand((1, h, t, c), dtype, device, _seed(spec, 6))
    da = _rand((1, h, t, c), dtype, device, _seed(spec, 7))
    dw = _rand((1, h, t, int(spec.get("K", 128))), dtype, device, _seed(spec, 8))
    du = _rand((1, h, t, int(spec.get("V", 128))), dtype, device, _seed(spec, 9))
    return k, v, beta, a, da, dw, du, g, int(spec.get("chunk_size", 64)), _cu(spec), _chunk_indices(spec)
...
def reference_outputs(spec: dict, low_marker: torch.Tensor) -> tuple[torch.Tensor, ...]:
    ...
    return (
        ...
        torch.zeros((b, h, t, v_dim), device=device, dtype=dtype),
        torch.zeros((b, h, t), device=device, dtype=_dtype(spec.get("beta_dtype", "fp32"))),
        torch.zeros((b, h, t), device=device, dtype=torch.float32),
    )
...
def run_npu(public_api: str, spec: dict, low_marker: torch.Tensor):
    ...
    return op(*args[:9], cu_seqlens=args[9], chunk_indices=args[10])
...
class FunctionApi(BaseApi):
    ...
    public_api = "prepare_wy_repr_bwd_full"
