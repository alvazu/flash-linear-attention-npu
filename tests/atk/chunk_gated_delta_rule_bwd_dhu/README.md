# ChunkGatedDeltaRuleBwdDhu ATK 精度测试归档

本目录归档 `chunk_gated_delta_rule_bwd_dhu` 算子的 ATK 看护资产。算子仅覆盖 A2
...
| 项 | 值 |
| --- | --- |
| 公开 Python API | `fla_npu.ops.ascendc.chunk_gated_delta_rule_bwd_dhu` |
| ACLNN 名称 | `ChunkGatedDeltaRuleBwdDhu` |
| 源算子 | `fla/ops/ascendc/gdn/chunk_gdn_bwd/chunk_gated_delta_rule_bwd_dhu` |
| SOC | `ascend910b`（A2） |
| route | `ascendc` |
...
分别验证两种 dtype 与两种序列组织下的 `dhu` 反传梯度。

## 2. 文件

```text
atk_chunk_gated_delta_rule_bwd_dhu.json
chunk_gated_delta_rule_bwd_dhu.yaml
executor_chunk_gated_delta_rule_bwd_dhu.py
gen_chunk_gated_delta_rule_bwd_dhu.py
README.md
```

`executor_chunk_gated_delta_rule_bwd_dhu.py` 注册为 `executor_chunk_gated_delta_rule_bwd_dhu`，复用
...
cd "$REPO_ROOT/test/chunk_gated_delta_rule_bwd_dhu"
atk --version
python -c 'import fla_npu; from fla_npu.ops.ascendc import chunk_gated_delta_rule_bwd_dhu; print(fla_npu.__file__)'
...
bash build.sh --pkg --soc=ascend910b \
  --vendor_name=fla_npu --ops=chunk_gated_delta_rule_bwd_dhu
```
...
atk node --name npu_dut --backend npu --devices 0 \
  ...
  node --name cpu_reference --backend cpu \
    --output_path ./atk_output/cpu_dual_reference \
  task \
    -c ./atk_chunk_gated_delta_rule_bwd_dhu.json \
    --task accuracy \
    --bm_device cpu \
    -p ./executor_chunk_gated_delta_rule_bwd_dhu.py \
    -s 0 \
    -e 2 \
    -sp \
    -mt 1 \
...
cd "$REPO_ROOT/test/chunk_gated_delta_rule_bwd_dhu"
atk case \
  -f ./chunk_gated_delta_rule_bwd_dhu.yaml \
  -p ./gen_chunk_gated_delta_rule_bwd_dhu.py \
  -dt 1 \
  -en 0 \
  -s 20260813

python3 ./gen_chunk_gated_delta_rule_bwd_dhu.py \
  --output ./atk_chunk_gated_delta_rule_bwd_dhu.generated.json \
  --summary
```
...
ct viz \
  ...
  --name chunk_gated_delta_rule_bwd_dhu_out \
  --spatial
```
...
| NPU 报算子不存在 | 未构建/安装匹配 SOC 的 OPP 包。按第 3.1 节构建 `ascend910b` 的 `chunk_gated_delta_rule_bwd_dhu` OPP。 |
...
| 生成结果与已评审 JSON 不一致 | 比较生成器、YAML 和 seed；不要静默覆盖 `atk_chunk_gated_delta_rule_bwd_dhu.json`。 |
