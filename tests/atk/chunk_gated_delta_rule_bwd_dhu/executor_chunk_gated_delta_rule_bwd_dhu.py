"""ATK executor for chunk_gated_delta_rule_bwd_dhu."""
...
def prepare_call_args(spec: dict, low_marker: torch.Tensor):
    ...
    q, k, _v, g, _beta = _layout_qkv(spec, device)
    b = int(spec.get("B", 1))
    ...
    do = _rand((b, hv, t, v_dim), dtype, device, _seed(spec, 7))
    dv = _rand((b, hv, t, v_dim), dtype, device, _seed(spec, 8))
    h0 = _rand((b, hv, chunks, k_dim, v_dim), dtype, device, _seed(spec, 9))
    dht = _rand((b, hv, chunks, k_dim, v_dim), dtype, device, _seed(spec, 10))
    scale = float(spec.get("scale", 1.0 / math.sqrt(k_dim)))
    return q, k, w, do, dv, scale, int(spec.get("chunk_size", 64)), g, None, h0, dht, _cu(spec), _chunk_indices(spec)
...
def reference_outputs(spec: dict, low_marker: torch.Tensor) -> tuple[torch.Tensor, ...]:
    ...
    chunks = _chunk_count(spec)
    return (
        torch.zeros((b, hv, chunks, k_dim, v_dim), device=device, dtype=dtype),
        torch.zeros((b, hv, chunks, k_dim, v_dim), device=device, dtype=dtype),
...
def run_npu(public_api: str, spec: dict, low_marker: torch.Tensor):
    ...
    return op(*args[:7], g=args[7], gK=args[8], h0=args[9], dht=args[10], cu_seqlens=args[11], chunk_indices=args[12])
...
class FunctionApi(BaseApi):
    ...
    public_api = "chunk_gated_delta_rule_bwd_dhu"
