# KdaGateCumsum ATK 精度测试归档

本目录归档 `kda_gate_cumsum` 算子的 ATK 看护资产。算子仅覆盖 A2
...
| 项 | 值 |
| --- | --- |
| 公开 Python API | `fla_npu.ops.ascendc.kda_gate_cumsum` |
| ACLNN 名称 | `KdaGateCumsum` |
| 源算子 | `fla/ops/ascendc/kda/kda_gate_cumsum` |
| SOC | `ascend910b`（A2） |
| route | `ascendc` |
...
| case ID | seed | B | HV | K | T | chunk_size | g_dtype | use_gate_in_kernel | varlen | cu_seqlens | explicit_chunk_indices | tags |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 0 | 20260813 | 1 | 8 | 128 | 128 | 64 | fp32 | true | false | "" | false | accuracy,smoke |
| 1 | 20260814 | 1 | 8 | 128 | 256 | 64 | bf16 | true | true | "0,128,256" | true | accuracy,smoke |

两条 case 共享 `B=1`、`HV=8`、`K=128`、`chunk_size=64`、`use_gate_in_kernel=true`。
case 0 覆盖固定长度、`g_dtype=fp32`；case 1 覆盖 varlen（`cu_seqlens="0,128,256"`）、
`g_dtype=bf16`、显式 chunk 索引，分别验证两种 gate dtype 与固定/varlen 序列下的
KDA gate 局部前缀和输出。

## 2. 文件

```text
atk_kda_gate_cumsum.json
kda_gate_cumsum.yaml
executor_kda_gate_cumsum.py
gen_kda_gate_cumsum.py
README.md
```

`executor_kda_gate_cumsum.py` 注册为 `executor_kda_gate_cumsum`，复用
...
cd "$REPO_ROOT/test/kda_gate_cumsum"
atk --version
python -c 'import fla_npu; from fla_npu.ops.ascendc import kda_gate_cumsum; print(fla_npu.__file__)'
...
bash build.sh --pkg --soc=ascend910b \
  --vendor_name=fla_npu --ops=kda_gate_cumsum
```
...
atk node --name npu_dut --backend npu --devices 0 \
  ...
  node --name cpu_reference --backend cpu \
    --output_path ./atk_output/cpu_dual_reference \
  task \
    -c ./atk_kda_gate_cumsum.json \
    --task accuracy \
    --bm_device cpu \
    -p ./executor_kda_gate_cumsum.py \
    -s 0 \
    -e 2 \
    -sp \
    -mt 1 \
...
cd "$REPO_ROOT/test/kda_gate_cumsum"
atk case \
  -f ./kda_gate_cumsum.yaml \
  -p ./gen_kda_gate_cumsum.py \
  -dt 1 \
  -en 0 \
  -s 20260813

python3 ./gen_kda_gate_cumsum.py \
  --output ./atk_kda_gate_cumsum.generated.json \
  --summary
```

相同 YAML、gen 和 seed 必须生成稳定 case ID 与稳定结构。生成后检查 ATK schema、
case 数量、SOC、route、g_dtype、chunk 边界、varlen 分布和覆盖摘要，不要静默覆盖已评审的
`atk_kda_gate_cumsum.json`。
...
| NPU 报算子不存在 | 未构建/安装匹配 SOC 的 OPP 包。按第 3.1 节构建 `ascend910b` 的 `kda_gate_cumsum` OPP。 |
...
| 生成结果与已评审 JSON 不一致 | 比较生成器、YAML 和 seed；不要静默覆盖 `atk_kda_gate_cumsum.json`。 |
