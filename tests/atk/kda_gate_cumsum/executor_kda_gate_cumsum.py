"""ATK executor for kda_gate_cumsum."""
...
def prepare_call_args(spec: dict, low_marker: torch.Tensor):
    ...
    hv = int(spec.get("HV", 8))
    k_dim = int(spec.get("K", 128))
    g = _normal((b, t, hv, k_dim), _dtype(spec.get("g_dtype", "fp32")), device, _seed(spec, 1), scale=0.02)
    a_log = _normal((hv,), torch.float32, device, _seed(spec, 2), scale=0.02)
    dt_bias = _normal((hv * k_dim,), torch.float32, device, _seed(spec, 3), scale=0.02)
    return g, int(spec.get("chunk_size", 64)), a_log, dt_bias, _cu(spec), _as_bool(spec.get("use_gate_in_kernel", True)), True, -5.0
...
def reference_outputs(spec: dict, low_marker: torch.Tensor) -> tuple[torch.Tensor, ...]:
    ...
    k_dim = int(spec.get("K", 128))
    g = _normal((b, t, hv, k_dim), _dtype(spec.get("g_dtype", "fp32")), device, _seed(spec, 1), scale=0.02)
    return (torch.cumsum(g.to(torch.float32), dim=1),)
...
def run_npu(public_api: str, spec: dict, low_marker: torch.Tensor):
    ...
    return op(args[0], args[1], A_log=args[2], dt_bias=args[3], cu_seqlens=args[4], use_gate_in_kernel=args[5], safe_gate=args[6], lower_bound=args[7])
...
@register("executor_kda_gate_cumsum")
class FunctionApi(BaseApi):
    op_name = "kda_gate_cumsum"
    public_api = "kda_gate_cumsum"
