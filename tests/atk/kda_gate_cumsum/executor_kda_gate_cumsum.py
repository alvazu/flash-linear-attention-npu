"""kda_gate_cumsum 的 ATK executor。
...
def build_inputs(spec: dict[str, Any], device: torch.device, high_precision: bool = False) -> dict[str, Any]:
    ...
    B, HV, T, K = (int(spec[x]) for x in ("B", "HV", "T", "K"))
    return {
        "g": _kda_gate((B, HV, T, K), dtype_name, calc_dtype, device, seed + 1),
        "chunk_size": int(spec["chunk_size"]),
    }


def _kda_gate_cumsum_ref(inputs):
    g = inputs["g"]
    calc = torch.float64 if g.dtype == torch.float64 else torch.float32
    out = torch.zeros_like(g, dtype=calc)
    for start, end in _chunks(g.shape[2], int(inputs["chunk_size"])):
        out[:, :, start:end, :] = torch.cumsum(g[:, :, start:end, :].to(calc) * _RCP_LN2, dim=2)
...
def run_cpu(spec: dict[str, Any], high_precision: bool = False):
    ...
    return _kda_gate_cumsum_ref(inputs)
...
def run_npu(spec: dict[str, Any], input_data: InputDataset):
    ...
    return ascendc.kda_gate_cumsum(inputs["g"], inputs["chunk_size"], A_log=None, dt_bias=None, cu_seqlens=None, use_gate_in_kernel=False, safe_gate=False, lower_bound=None)
