"""ATK executor for recompute_w_u_fwd."""
...
def prepare_call_args(spec: dict, low_marker: torch.Tensor):
    ...
    a = _rand((int(spec.get("B", 1)), int(spec.get("HV", spec.get("H", 4))), int(spec.get("T", 128)), int(spec.get("chunk_size", 64))), _dtype(spec.get("dtype", "bf16")), device, _seed(spec, 6))
    return k, v, beta, a, int(spec.get("chunk_size", 64)), g, None, _cu(spec), _chunk_indices(spec)
...
def run_npu(public_api: str, spec: dict, low_marker: torch.Tensor):
    ...
    return op(*args[:5], g=args[5], gk=args[6], cu_seqlens=args[7], chunk_indices=args[8])
