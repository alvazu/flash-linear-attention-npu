# CausalConv1d ATK 精度测试归档

本目录归档 `causal_conv1d` 算子的 ATK 看护资产。算子仅覆盖 A2
...
| 项 | 值 |
| --- | --- |
| 公开 Python API | `fla_npu.ops.ascendc.causal_conv1d` |
| ACLNN 名称 | `CausalConv1d` |
| 源算子 | `fla/ops/ascendc/gdn/gdn_preprocess/causal_conv1d` |
| SOC | `ascend910b`（A2） |
| route | `ascendc` |
...
| case ID | seed | B | T | D | W | dtype | has_bias | cu_seqlens | explicit_chunk_indices | tags |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 0 | 20260813 | 2 | 128 | 64 | 4 | bf16 | true | "" | false | accuracy,smoke |
| 1 | 20260814 | 1 | 256 | 128 | 4 | fp16 | false | "" | false | accuracy,smoke |

两条 case 均为固定长度（`cu_seqlens=""`）、`head_num=0`、`run_mode=0`。case 0
覆盖 BF16 带偏置，case 1 覆盖 FP16 无偏置，分别验证两种 dtype 下的因果卷积前向。

## 2. 文件

```text
atk_causal_conv1d.json
causal_conv1d.yaml
executor_causal_conv1d.py
gen_causal_conv1d.py
README.md
```

`executor_causal_conv1d.py` 注册为 `executor_causal_conv1d`，复用
...
cd "$REPO_ROOT/test/causal_conv1d"
atk --version
python -c 'import fla_npu; from fla_npu.ops.ascendc import causal_conv1d; print(fla_npu.__file__)'
...
bash build.sh --pkg --soc=ascend910b \
  --vendor_name=fla_npu --ops=causal_conv1d
```
...
atk node --name npu_dut --backend npu --devices 0 \
  ...
  node --name cpu_reference --backend cpu \
    --output_path ./atk_output/cpu_dual_reference \
  task \
    -c ./atk_causal_conv1d.json \
    --task accuracy \
    --bm_device cpu \
    -p ./executor_causal_conv1d.py \
    -s 0 \
    -e 2 \
    -sp \
    -mt 1 \
...
cd "$REPO_ROOT/test/causal_conv1d"
atk case \
  -f ./causal_conv1d.yaml \
  -p ./gen_causal_conv1d.py \
  -dt 1 \
  -en 0 \
  -s 20260813

python3 ./gen_causal_conv1d.py \
  --output ./atk_causal_conv1d.generated.json \
  --summary
```
...
| NPU 报算子不存在 | 未构建/安装匹配 SOC 的 OPP 包。按第 3.1 节构建 `ascend910b` 的 `causal_conv1d` OPP。 |
...
| 生成结果与已评审 JSON 不一致 | 比较生成器、YAML 和 seed；不要静默覆盖 `atk_causal_conv1d.json`。 |
