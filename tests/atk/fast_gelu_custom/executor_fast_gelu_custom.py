"""ATK executor for fast_gelu_custom."""
...
def prepare_call_args(spec: dict, low_marker: torch.Tensor):
    ...
    return (_rand(spec["shape"], _dtype(spec["dtype"]), device, _seed(spec, 1), low=-3.0, high=3.0),)


def reference_outputs(spec: dict, low_marker: torch.Tensor) -> tuple[torch.Tensor, ...]:
    device = low_marker.device
    x = _rand(spec["shape"], _dtype(spec["dtype"]), device, _seed(spec, 1), low=-3.0, high=3.0)
...
@register("executor_fast_gelu_custom")
class FunctionApi(BaseApi):
    op_name = "fast_gelu_custom"
    public_api = "fast_gelu_custom"
